...
const V = 'jh-v2', SHELL = ['/', '/me', '/app.css', '/icon.svg', '/icon-192.png', '/manifest.webmanifest'];
...
self.addEventListener('fetch', (e) => {
  ...
  e.respondWith(fetch(e.request).then((r) => { if (r.ok) { const c = r.clone(); caches.open(V).then((x) => x.put(e.request, c)); } return r; })
    .catch(() => caches.match(e.request).then((r) => r || caches.match(u.pathname.startsWith('/me') ? '/me' : '/'))));
});
